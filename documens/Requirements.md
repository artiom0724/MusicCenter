...
3.1.1 Возможности идентификации:
1. Пользователю будет предоставлена возможность авторизации.
2. Незарегистрированному польвателю будет предоставлена возможность регистрации.

3.1.2 Авторизация:
1. Возможность авторизации с помощью одной из социальных сетей. Пользователь будет переадресован на страницу подтвеждения
   авторизации при помощи выбранной социальной сети, при успешной авторизации пользователь будет 
   переадресован на главную страницу.
2. Зарегистрированные пользователи смогут авторизаваться, после чего будут проверенны введенные пользователем данные на корректность, в    случае успеха польватель будет переадресован на главную страницу.

3.1.3 Регистрация:
1. Пользователи смогут зарегистрироваться, после чего будут проверенны введенные пользователем данные на корректность, в случае 
   успеха будет создана новая учетная зпись и пользователь будет переадресован на главную страницу.

3.1.4 Проигрывание музыки(Плейер):
1. Запоминание последней прослушаной музыки.
2. Отправка произведений в плейлист и проигрывание тогда, когда до него дойдёт очередь.
3. Выбор жанра, заполнение полей "Artist", "Album" позволит произвести поиск по соответствующим категориям.

3.1.5 Редактирование профиля:
1. Пользователь может изменить свой пароль.
2.Сохраниение песен в "Favorites" или в "Listened" пользователь будет перенаправлен на страницу плейера, где соответствующая песня будет иметь возможность быть проиграной.
